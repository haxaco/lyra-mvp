...
export async function PATCH(req: NextRequest, { params }: { params: { id: string }}) {
  try {
    ...
    if (Object.keys(updates).length) {
      const { error: uErr } = await supa.from("playlists").update(updates).eq("id", params.id);
      if (uErr) throw uErr;
    }

    if (Array.isArray(body?.trackIds)) {
      // Reset items: delete then reinsert with new order
      const { error: dErr } = await supa.from("playlist_items").delete().eq("playlist_id", params.id);
      if (dErr) throw dErr;
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string }}) {
  try {
    ...
    const { error } = await supa.from("playlists").delete().eq("id", params.id);
